...
public class SinglePlayerProcessor {
    ...
    public SinglePlayerProcessor(Footballer footballer, FootballerDetails currentData, ProcessedPlayer previousData) {
    ...
    public ProcessedPlayer process() {
        ...
        printMatchEvents(currentPlayerData.events);
    ...
    private static void addNewEvents(List<MatchEvent> diff, FootballerScoreDetailElement detailsDiff, Footballer footballer, FootballerScoreDetailElement currentDetail) {
        ...
        if (detailsDiff.assists.value != 0) {
            diff.add(createMatchEvent(time, MatchEventType.ASSIST, footballer, detailsDiff.assists.value, detailsDiff.assists.points));
        }
        if (detailsDiff.goals_scored.value != 0) {
            diff.add(createMatchEvent(time, MatchEventType.GOAL, footballer, detailsDiff.goals_scored.value, detailsDiff.goals_scored.points));
        }
        if (detailsDiff.minutes.points != 0) {
            diff.add(createMatchEvent(time, MatchEventType.MINUTES_PLAYED, footballer, currentDetail.minutes.value, detailsDiff.minutes.points));
        }
        if (detailsDiff.clean_sheets.value != 0) {
            diff.add(createMatchEvent(time, MatchEventType.CLEAN_SHEET, footballer, detailsDiff.clean_sheets.value, detailsDiff.clean_sheets.points));
        }
        if (detailsDiff.bonus.value != 0) {
            diff.add(createMatchEvent(time, MatchEventType.BONUS, footballer, detailsDiff.bonus.value, detailsDiff.bonus.points));
        }
        if (detailsDiff.yellow_cards.value != 0) {
            diff.add(createMatchEvent(time, MatchEventType.YELLOW_CARD, footballer, detailsDiff.yellow_cards.value, detailsDiff.yellow_cards.points));
        }
        if (detailsDiff.red_cards.value != 0) {
            diff.add(createMatchEvent(time, MatchEventType.RED_CARD, footballer, detailsDiff.red_cards.value, detailsDiff.red_cards.points));
        }
        if (detailsDiff.penalty_misses.value != 0) {
            diff.add(createMatchEvent(time, MatchEventType.PENALTY_MISS, footballer, detailsDiff.penalty_misses.value, detailsDiff.penalty_misses.points));
        }
        if (detailsDiff.goals_conceded.value != 0) {
            diff.add(createMatchEvent(time, MatchEventType.GOALS_CONCEDED, footballer, detailsDiff.goals_conceded.value, detailsDiff.goals_conceded.points));
        }
        if (detailsDiff.saves.value != 0) {
            diff.add(createMatchEvent(time, MatchEventType.SAVES, footballer, detailsDiff.saves.value, detailsDiff.saves.points));
        }
        if (detailsDiff.penalty_saves.value != 0) {
            diff.add(createMatchEvent(time, MatchEventType.PENALTY_SAVES, footballer, detailsDiff.penalty_saves.value, detailsDiff.penalty_saves.points));
        }
        if (detailsDiff.own_goals.value != 0) {
            diff.add(createMatchEvent(time, MatchEventType.OWN_GOALS, footballer, detailsDiff.own_goals.value, detailsDiff.own_goals.points));
        }
    }
